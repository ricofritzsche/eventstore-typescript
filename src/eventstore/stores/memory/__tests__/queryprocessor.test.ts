...
describe('Query processing', () => {
    ...
    describe('querying', () => { 
        it('no records, not results', async () => {
            ...
            const query = createQuery(createFilter(["test1"]));
            
            const result = processQuery(events, query)
        ...
        it('filer by several event type', async () => {
            ...
            const events = es.eventRecords;
            const query = createQuery(createFilter(["test1", "test3"]))
        ...
        it('filer by event type and simple payload', async () => {
            ...
            const events = es.eventRecords;
            const query = createQuery(createFilter(["test1"], [{id: 2}]))
        ...
        it('filter by event type and nested payload', async () => {
            ...
            const events = es.eventRecords;
            const query = createQuery(createFilter(["test1"], [{contact: { address: { zip: "2777" } } }]))
        ...
        it('filter by event type and nested payload with multiple predicates', async () => {
            ...
            const events = es.eventRecords;
            const query = createQuery(createFilter(["test1"], [{contact: { address: { zip: "2777" } } }, {contact: { address: { zip: "1234" } } }]))
        ...
        it('filter with multiple event types and nested payload with multiple predicates', async () => {
            ...
            const events = es.eventRecords;
            const query = createQuery(createFilter(["test1", "test3"], [{contact: { address: { zip: "2777" } } }, {contact: { address: { zip: "1234" } } }, { id: 99 }]))
        ...
        it('query with multiple filters with only event types', async () => {
            es.append([{
                eventType: 'test1',
                payload: { id: 1, contact: { address: { zip: "1234" } } }
            }, {
            ...
            const events = es.eventRecords;
            const query = createQuery(createFilter(["test1"]), createFilter(["test3"]))
        ...
        it('query with multiple filters and payloads', async () => {
            es.append([{
                eventType: 'test1',
                payload: { id: 1 }
            }, {
            ...
            const events = es.eventRecords;
            const query = createQuery(createFilter(["test1"], [{id: 99}]), createFilter(["test2"], [{id: 98}]))
            ...
            expect(result[0]?.eventType).toBe("test1");
            expect(result[0]?.payload.id).toBe(99);

            expect(result[1]?.eventType).toBe("test2");
            expect(result[1]?.payload.id).toBe(98);
        });
