...
describe('Optimistic Locking CTE Condition', () => {
  ...
  it('should fail when sequence number does not match (CTE condition)', async () => {
    ...
    // Try to append with an outdated sequence number (should fail)
  ...
  it('should handle concurrent modifications correctly', async () => {
    ...
    // Simulate a concurrent scenario:
    ...
    // 2. The first process successfully appends using its context sequence number
  ...
  it('should work with payload predicates in CTE condition', async () => {
    ...
    const filter = createQuery(createFilter([eventType], [{ accountId: accountId }]));

    // Insert event for a different account (should not affect our context)
    const otherFilter = createQuery(createFilter([eventType], [{ accountId: 'other-account' }]));
